Shader "Cook-Torrance BRDF"
{
    Properties
    {
        ...
        [Header(PBR)]
        _Tint("Diffuse Color",Color)=(1,1,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Energy conservation (Geometry Term)
            float geometrySmith(float NdotV, float k)
            {
                float nom = NdotV;
            ...
            {
                float squareRoughness = pow(Roughness, 2);
                float k = pow(squareRoughness + 1, 2) / 8;
                float ggx1 = geometrySmith(NdotV, k); // NdotV occlusion term
                float ggx2 = geometrySmith(NdotL, k); // NdotL shadows term
                return ggx1 * ggx2;
            }

            // Use approx function instead of sampling a Schlick BRDF LUT, see Unreal Engine 4 tech blog.
            ...
            // Mipmap of Cubemap
            half MipCubeMap(half perceptualRoughness)
            {
                // Mip-Level
            ...
            {
                half D = GGXTerm (NdotH, roughness);    // Normal Dist.
                half G = geometrySmith(NdotV, NdotL, roughness);    // Geometry
                half3 F = fresnelSchlick(F0, VdotH);    // Fresnel
                half3 BRDF = (D * G * F * 0.25) / (NdotV * NdotL);
                return BRDF;
            }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // Use 0.00001 to void zero value
                float ndl = max(1e-5,saturate(dot(bump,lightDir))); 
                ...
                // Direct diffuse    
				half3 kd = OneMinusReflectivityFromMetallic(metallic);   // Unity's 1-metallic, but with adjustment related to colorspace.
                half3 DiffcuseResult = kd * albedo.rgb;

                half3 DirectLightResult =  (DiffcuseResult + SpecularResult* UNITY_PI) * _LightColor0 * ndl;  

                // Indirect diffuse
                half3 ambient_contrib = ShadeSH12Order(float4(bump, 1));   // 12 spherical harmonics
                half3 ambient = 0.03 * albedo.rgb;    // Ambient factor     
                half3 iblDiffuse = max(half3(0, 0, 0), ambient.rgb + ambient_contrib);
                half3 Flast = FresnelSchlickRoughness(ndv,F0, roughness); 
                half3 kdLast = (1 - Flast) * (1 - metallic);          // Diffuse factor 
                half3 iblDiffuseResult = iblDiffuse * kdLast * albedo.rgb;

                // Indirect lighting caused by environment mapping
                half mip = MipCubeMap(perceptualRoughness);  
